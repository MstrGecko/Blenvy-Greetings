...
def process_tuples(registry, definition, prefixItems, update, nesting_long_names=[], depth=0, processed_types=None):
    ...
    for index, item in enumerate(prefixItems):
        ...
        if ref_name in type_infos:
            ...
            if is_value_type:
                ...
                original_long_name = original["long_name"]
                (sub_component_group, _) = process_component.process_component(registry, original, update, {"nested": True, "long_name": original_long_name}, nesting_long_names=nesting_long_names+[property_name], depth=depth+1, processed_types=processed_types)
